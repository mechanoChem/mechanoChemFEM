...
{	
  ...
  //std::cout << "--b0-0 (primary_dof)--: "  << std::endl;
  ...
  int ind_Electrode_potential = 1;
  int ind_Displacement_sd_1 = 2;
  int ind_Displacement_sd_2 = 3;
  int ind_Displacement_wd = 4;
  ...
  //{ // for electrode
    ...
    Sacado::Fad::DFad<double> c_li_ave=0.0, c_li_plus_ave=0.0, phi_s_ave=0.0;
    {
      int _count_plus = 0;
      for (unsigned i = 0; i < cell_SDdata[cell_id].lnode_plus.size(); ++i) {
  ...
  for (unsigned int q = 0; q < n_q_points; ++q) {
    ...
    lithium.set_diffusion_reaction_term_interface(diffu_Lithium_tilde, react_Lithium_tilde, c_1_tilde_grad_Lithium, pressure_old[cell_id]);
		//diffu_Lithium_tilde=table_scaling<2, Sacado::Fad::DFad<double>, Sacado::Fad::DFad<double>>(diffu_Lithium_tilde,0.0);
		//diffu_Lithium=table_scaling<2, Sacado::Fad::DFad<double>, Sacado::Fad::DFad<double>>(diffu_Lithium,0.0);
		//react_Lithium=table_scaling<1, Sacado::Fad::DFad<double>>(react_Lithium,0.0);
		//react_Lithium_tilde=table_scaling<1, Sacado::Fad::DFad<double>>(react_Lithium_tilde,0.0);
  ...
  //---------- compute Rcc-------------------
    ...
    for (unsigned int q = 0; q < n_q_points; ++q) {
      for (unsigned i = 0; i < this_dof_local_index_Electrode_potential.size(); ++i) {
        int plus_node = this_dof_local_index_Electrode_potential[i];
        for (unsigned int j = 0; j < dim; j++) {
            Rcc_Electrode_potential[i] += -fe_values.shape_grad(plus_node, q)[j]*field_Electrode_potential[q][j]*fe_values.JxW(q); // oscillation
            //std::cout << " q " << q << " j " << j << " plus_node " << plus_node << " shape_grad " << fe_values.shape_grad(plus_node, q)[j] << " field_potential " << field_Electrode_potential[q][j] << std::endl;
        }
      }
    }
  ...
  for (unsigned int q = 0; q < n_q_points; ++q) {
    ...
    if (center[orientation]>separator_line){ // only positive line will fracture.
    	if (TN_at_gp[q]/10000.0 >  (*displacement.params_json)["Mechanics"]["MaximumStressN"] and not cell_SDdata[cell_id].is_fractured and this->is_new_step[cell_id])
    	{
