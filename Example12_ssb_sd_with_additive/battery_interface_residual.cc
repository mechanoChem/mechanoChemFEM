...
{	
  ...
  //{ // for electrode
    ...
    //std::cout << " center " << center << " jn " << jn.val() <<  " T " << Temp 
      //<< " c_li " <<  c_li_tld.val() << " c_li+ " <<  c_li_plus_tld.val() 
      //<< " phi_s " <<  phi_s_tld.val()  << " phi_e " << phi_e_tld.val() 
      //<< " domain " <<   domainflag << std::endl;
  ...
  //if(battery_fields.active_fields_index["Electrolyte_potential"]>-1) phi_e.r_get_residual_with_interface(cell, fe_values, R, ULocal, ULocalConv, cell_SDdata);
    ...
    for (unsigned int i = 0; i < dofs_per_cell; ++i) {
      ...
      if (R[i].val() != R[i].val()) 
      {
        //std::cout << "compute residual at the active SE interface " << cell_id  << " center: " << center << std::endl;
