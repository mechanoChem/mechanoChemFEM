...
{	
	...
	//int domainflag=5; // id for li-metal-interface, so Usc=0.0
	int domainflag=-1;
  ...
  for (unsigned int q = 0; q < n_q_points; ++q) {
	  ...
	  dealii::Table<3, Sacado::Fad::DFad<double> > P(n_q_points,dim,dim);
    {
      ...
      double swell_ratio = (*params_json)["Mechanics"]["SwellRatioAnode"];
  ...
  //if(battery_fields.active_fields_index["Electrolyte_potential"]>-1) phi_e.r_get_residual_with_interface(cell, fe_values, R, ULocal, ULocalConv, cell_SDdata);
    ...
    for (unsigned int i = 0; i < dofs_per_cell; ++i) {
      ...
      if (R[i].val() != R[i].val()) 
      {
        //std::cout << "compute residual at the li metal interface " << cell_id  << " center: " << center << std::endl;
