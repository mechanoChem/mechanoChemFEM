...
T ElectricChemo<dim, T>::formula_j0(T c_li, T c_li_plus, int domainflag)
{
	...
	else if(domainflag==5) // li -metal interface
  {k=k_neg;alpha=alpha_neg;c1max=c_max_neg; }


	UnitC_surface=c_li/c1max;
  j0=k*c1max*std::pow(c_li_plus,alpha)*std::pow((1-UnitC_surface),alpha)*std::pow(UnitC_surface,(1-alpha));

  //std::cout << " k " << k << " c1max " << c1max << " c_li_plus " << c_li_plus << " UnitC_surface " << UnitC_surface << " j0 " << j0.val() 
    //<< std::pow(c_li_plus,alpha)
    //<< std::pow((1-UnitC_surface),alpha)
    //<< std::pow(UnitC_surface,(1-alpha))
    //<< std::endl;
...
T ElectricChemo<dim, T>::formula_jn(T Temp, T c_li, T c_li_plus, T phi_s, T phi_e, int domainflag)
{
	...
	if(domainflag==5) // li-metal interface
  {
		double k, alpha;
		T j0, Usc, UnitC;
		k=k_neg; alpha=alpha_neg; UnitC=c_li/c_max_neg; 
    ...
    //std::cout << "j0" << j0 << std::endl;
        ...
        //std::cout << " j0 " << j0.val() << " eta " << eta.val() << " phi_s " << phi_s.val() << " phi_e "  << phi_e.val()  << " Usc " <<  Usc.val() << std::endl;
		jn=j0*(exp(alpha*F/Rr/Temp*eta)-exp(-alpha*F/Rr/Temp*eta));
	}
	return jn;
}
