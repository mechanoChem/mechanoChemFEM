...
battery<dim>::battery(std::string parameter_file_Dir)
{
	...
	this->declare_parameters_mechanoChemFEM();
...
template <int dim>
void battery<dim>::declare_parameters_batteryFields()
{
...
void battery<dim>::setMultDomain()
{
  ...
  for (typename Triangulation<dim>::active_cell_iterator cell = this->dof_handler.begin_active(); cell != this->dof_handler.end(); ++cell){
    Point<dim> cell_center = cell->center();
		if(cell_center[0]<X_0+neg_electrode_len) cell->set_material_id(pos_electrode_domain_id);
		else if(cell_center[0]<X_0+neg_electrode_len+separator_len) cell->set_material_id(separator_domain_id);
		else cell->set_material_id(neg_electrode_domain_id);
	}
