...
void CahnHilliard<dim>::get_residual(const typename hp::DoFHandler<dim>::active_cell_iterator &cell, const FEValues<dim>& fe_values, Table<1, Sacado::Fad::DFad<double> >& R, Table<1, Sacado::Fad::DFad<double>>& ULocal, Table<1, double >& ULocalConv)
{
  ...
  params->leave_subsection();
  //double jn = 0.;
  //double jn = -0.5; //this->current_time
  double jn;
  if (this->current_time <= 500){
    jn = -5.0/115200.;
  }
  else if (this->current_time <= 800){
    jn = 8./115200.;
  }
  else if (this->current_time <= 1300){
    jn = -5.0/115200.;
  }
  else if (this->current_time <= 2000){
    jn = 2.5/115200.;
  }
  else{
    jn = 0.;
  ...
  for(unsigned int q=0; q<n_q_points; q++){
    M = 0.25*1.e-2*std::exp(-274.0*(1.05-c_1[q])*(0.47-c_1[q])*(1.-c_1[q]))*c_1[q]/(kB*T); //micron^2/(eV*s*unit_cell)
    for (int i=0; i<dim; ++i){
...
void CahnHilliard<dim>::solve_ibvp()
{
  ...
  bool multiple = (std::fmod(this->current_time,std::pow(10.,0.25)*this->current_dt) < 1.e-8);
  //if((iter_count>4) && multiple && (this->current_dt < 1.)) {
  if((iter_count>4) && (this->current_dt < 1.)) {
