...
void CahnHilliard<dim>::make_grid()
{
  ...
  std::ofstream fout("initial_mesh.msh");
  GridOut().write_msh(this->triangulation,fout);
  // */
...
void CahnHilliard<dim>::refine_grid_once(){
  ...
  //GridRefinement::refine_and_coarsen_fixed_fraction (hpFEM<dim>::triangulation, estimated_error_per_cell, 0.1, 0.2);
